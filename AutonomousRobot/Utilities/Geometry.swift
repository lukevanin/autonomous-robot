...
public extension SCNGeometry {
    ...
    static func from(_ arGeometry: ARMeshGeometry, ofType type: ARMeshClassification) -> SCNGeometry? {
        ...
        let vertices = usedVertexIndices.map { (vertexIndex: UInt32) -> SCNVector3 in
            let (x, y, z) = arGeometry.vertex(at: vertexIndex)
            return SCNVector3Make(x, y, z)
        }
        let normals = usedVertexIndices.map { (vertexIndex) -> SCNVector3 in
            let (x, y, z) = arGeometry.normal(at: UInt32(vertexIndex))
            return SCNVector3Make(x, y, z)
        }
        ...
        let bounds = vertices.reduce((SCNVector3Zero, SCNVector3Zero)) { (bbox, vertex) -> (SCNVector3, SCNVector3) in
            return (
                SCNVector3Make(.minimum(bbox.0.x, vertex.x), .minimum(bbox.0.y, vertex.y), .minimum(bbox.0.z, vertex.z)),
                SCNVector3Make(.maximum(bbox.1.x, vertex.x), .maximum(bbox.1.y, vertex.y), .maximum(bbox.1.z, vertex.z))
            )
        }
        let size = SCNVector3Make(bounds.1.x - bounds.0.x, bounds.1.y - bounds.0.y, bounds.1.z - bounds.0.z)
        ...
        return SCNGeometry(sources: [verticesSource, normalsSource, texcoordsSource], elements: [faces])
    }
    ...
    static func normalForest(from arGeometry: ARMeshGeometry) -> SCNGeometry {
        let vertices = (0..<arGeometry.vertices.count).map { (vertexIdx: Int) -> SCNVector3 in
            let (x, y, z) = arGeometry.vertex(at: UInt32(vertexIdx))
            return SCNVector3(x: x, y: y, z: z)
        }
        let normalVertices = (0..<arGeometry.normals.count).map { (normalIdx: Int) -> SCNVector3 in
            let (x, y, z) = arGeometry.normal(at: UInt32(normalIdx))
            let startVertex = vertices[normalIdx]
            let normalVector = SCNVector3(x: x, y: y, z: z).rescaled(to: 0.04) // 4 cm
            return SCNVector3(x: startVertex.x + normalVector.x, y: startVertex.y + normalVector.y, z: startVertex.z + normalVector.z)
        }
        ...
        let forest = SCNGeometry(sources: [vertexSource], elements: [linesSource])
        return forest
    }
}
